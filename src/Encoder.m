       ...
       didChangePreset:(BOOL)changePreset
{
    if (self = [super init]) {
        ...
        NSDictionary *sourcePixelBufferAttributesDictionary = [NSDictionary dictionaryWithObjectsAndKeys:
                                                               ...
                                                               [NSNumber numberWithInt:(int32_t)width], kCVPixelBufferWidthKey,
                                                               [NSNumber numberWithInt:(int32_t)height], kCVPixelBufferHeightKey,
                                                               nil];
        
        videoPixelBufferAdaptor = [AVAssetWriterInputPixelBufferAdaptor assetWriterInputPixelBufferAdaptorWithAssetWriterInput:video sourcePixelBufferAttributes:sourcePixelBufferAttributesDictionary];
        [videoPixelBufferAdaptor retain];
...
- (CVPixelBufferRef)pixelBufferFromPool
{
    CVPixelBufferRef pixelBuffer;
    if (kCVReturnSuccess == CVPixelBufferPoolCreatePixelBuffer(kCFAllocatorDefault, videoPixelBufferAdaptor.pixelBufferPool, &pixelBuffer)) {
        return pixelBuffer;
    } else {
        return nil;
    }
...
- (void)appendVideoSample:(CVPixelBufferRef)pixelBuffer
                      pts:(CMTime)ptime
{
    ...
    if (start) {
        ...
        [writer startSessionAtSourceTime:ptime];
    }
    
    AVAssetWriterInput *target = video;
    
    if ([target isReadyForMoreMediaData]) {
        [videoPixelBufferAdaptor appendPixelBuffer:pixelBuffer withPresentationTime:ptime];
    }
    
    [state unlock];
    
    int64_t pts = ptime.value / 1000000;
...
- (void)appendAudioSample:(CMSampleBufferRef)sampleBuffer
{
    ...
    AVAssetWriterInput *target = audio;
