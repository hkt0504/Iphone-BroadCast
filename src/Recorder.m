...
- (id)init {
    ...
    if ([TrialManager checkTrial]) {
        ...
        if (self = [super init]) {
            ...
            ciContext = [CIContext contextWithEAGLContext:glContext options:options];
            [ciContext retain];
             ...
             previewOrientation: (Video_Orientation)previewOrientation
{
    ...
    if (self = [self init]) {
        ...
        dispatch_queue_t sampleQueue = dispatch_queue_create("com.rtmp_samples", NULL);
...
//            }
            ...
            adaptiveManager = [[NetworkAdaptiveManager alloc] init];
            [adaptiveManager setRecorder: self];
            [adaptiveManager initializeManager];
        }
        ...
        @try {
            ...
            while (state == STATE_PLAYING && ! _noInternet) {
                ...
                @try {
                    ...
                    while ([rtmp writePacket:mov andSendVideo:sendVideo date: date newSize: newSessionPreset audioCodecContext: audioCodecContext]);
                }
                @finally {
                ...
                toEncoder:(Encoder *)encoder
{
    ...
    CGSize outputSize = [encoder outputVideoSize];
    
    // Create pixel buffer.
    CVPixelBufferRef pixelBuffer = [encoder pixelBufferFromPool];
    if (pixelBuffer == nil) {
        return;
    }
    ...
    if (true) {
        CIImage *rotatedImage = [ciImage imageByApplyingTransform:CGAffineTransformMakeRotation([MotionProvider sharedProvider].tiltAngle)];
        
        CGRect extent = [rotatedImage extent];
        cropRect = CGRectMake(extent.origin.x + (extent.size.width - outputSize.width) / 2, extent.origin.y + (extent.size.height - outputSize.height) / 2, outputSize.width, outputSize.height);
        
        midImage = rotatedImage;
        
    }

    CIFilter *cropFilter = [CIFilter filterWithName:@"CICrop"];
    [cropFilter setValue:midImage forKey:@"inputImage"];
    [cropFilter setValue:[CIVector vectorWithCGRect:cropRect] forKey:@"inputRectangle"];
    
    CIImage *croppedImage = cropFilter.outputImage;
    
    CIImage *resultImage = [croppedImage imageByApplyingTransform:CGAffineTransformMakeTranslation(-croppedImage.extent.origin.x, -croppedImage.extent.origin.y)];
    
    CVPixelBufferLockBaseAddress(pixelBuffer, 0);
    [ciContext render:resultImage toCVPixelBuffer:pixelBuffer];
    CVPixelBufferUnlockBaseAddress(pixelBuffer, 0);
    
    CVPixelBufferUnlockBaseAddress(imageBuffer, 0);

    CMTime ptime = CMSampleBufferGetPresentationTimeStamp(sampleBuffer);
    
    [encoder appendVideoSample:pixelBuffer pts:ptime];
                ...
                toEncoder:(Encoder *)encoder
{
    ...
    [encoder appendAudioSample:sampleBuffer];
}
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    dispatch_async(movieWritingQueue, ^{
        ...
        if (state == STATE_COLLECTIONG_EXTRA_DATA && captureOutput == audioOutput) {
            ...
            [self appendAudioSample:sampleBuffer toEncoder:audioExtraDataEncoder];
            ...
            if (! isAudio) {
                ...
                if (dropFrames) {
                    ...
                    if (frameCounter < frameDroppingFrequency) {
                        // [rearEncoder appendSample:sampleBuffer isAudio: NO];
                        [self appendVideoSample:sampleBuffer toEncoder:rearEncoder];
                    }
                    else {
                        //NSLog(@"drop frame------------------");
                        frameCounter = 0;
...
- (void)start {
    ...
    switch (type) {
        case LIVE_RECORDER:
            ...
            @try {
                rtmp = [[Streamer alloc] initWithUrl: destination
                                    andAudioRate: theAudioRate
                                    bufferLength: BUFFER_LENGTH
                                  adaptToNetwork: autoAdaptToNetwork
                                callbackListener: _delegate];
            ...
            rtmp.audioQueue = audioEncoder.audioQueue;
            rtmp.audioOffsetDelegate = audioEncoder;
...
- (void)saveVideoToCameralRoll:(BOOL)save
{
    if (state == STATE_STOPPED) {
        ...
        if (_delegate) {
            [_delegate streamingStateChanged: StreamingState_Warning withMessage: @"Cannot make modifications on the \"save video\" manager while streaming"];
        }
    }
}
...
- (void)autoAdaptToNetwork:(BOOL)adapt
{
    if (state == STATE_STOPPED) {
        ...
        if (_delegate) {
            [_delegate streamingStateChanged: StreamingState_Warning withMessage: @"Cannot make modifications on the \"auto adaptive\" manager while streaming"];
        }
    }
}
...
{
    if (rtmp) {
        return rtmp.getUploadBandwidth;
    }
    else return 0;
}
